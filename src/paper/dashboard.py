"""
Static HTML dashboard for paper trading signals — Polymarket.
"""
...
def generate_html(output_path: str = None) -> str:
    ...
    for s in signals:
        ...
        <tr>
            <td>{ts}</td>
            <td title="{s['market_id'][:20]}">{_trunc(s['market_title'], 50)}</td>
            <td>{s['side']}</td>
...
<title>Polymarket AI — Paper Trading Dashboard</title>
...
<body>
<h1>Polymarket AI — Paper Trading Dashboard</h1>
<div class="mode-banner">PAPER MODE — All positions and P&amp;L are <strong>simulated</strong>.</div>
<p class="subtitle">Gemini Flash AI Prediction Bot — Updated {now}</p>
...
<div class="stats">
  ...
  <div class="stat"><div class="value">${stats['avg_return']:.4f}</div><div class="label">Avg Return</div></div>
  <div class="stat"><div class="value pos">${stats['best_trade']:.2f}</div><div class="label">Best</div></div>
  <div class="stat"><div class="value neg">${stats['worst_trade']:.2f}</div><div class="label">Worst</div></div>
</div>
...
<tbody>{rows_html if rows_html else '<tr><td colspan="9" style="text-align:center;padding:2rem;color:#8b949e;">No signals yet.</td></tr>'}
</tbody>
</table>
</div>

<footer>Polymarket AI Prediction Bot — Paper trading — Not financial advice</footer>
